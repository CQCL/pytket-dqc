...
def to_bipartite(circuit):
    ...
    # Populate the IndexedCommands list for each ExtendedQubit
    for indexed_command in indexed_commands:
        for qubit in indexed_command.command.qubits:
            extended_qubits[qubit].add_indexed_command(indexed_command)

    # Create all the vertices on each qubit
    for extended_qubit in extended_qubits.values():
        for indexed_command in extended_qubit.get_indexed_commands():
            if indexed_command.is_packable() and indexed_command.is_local(): # Local packable gate
                vertex = extended_qubit.last_used_vertex
                indexed_command.add_vertex(vertex)
                vertex.add_indexed_command(indexed_command)
            
            elif indexed_command.is_1q(): # 1 qubit non-(anti)diagonal gate
                extended_qubit.close_all_vertices()
                new_vertex = CommandVertex(next_vertex_index, extended_qubit)
                indexed_command.add_vertex(new_vertex)
                new_vertex.add_indexed_command(indexed_command)
                extended_qubit.add_vertex(new_vertex)
                next_vertex_index += 1

            else: # Non-local CZ gate
                currently_connected_servers = extended_qubit.get_currently_connected_servers()
                if indexed_command.other_arg_server_num(extended_qubit.qubit) in currently_connected_servers.keys():
                    ...
                    vertex = list(currently_connected_servers[indexed_command.other_arg_server_num(extended_qubit.qubit)])[0]
                    ...
                    if extended_qubit.last_used_vertex.is_connected():
                        ...
                        vertex = extended_qubit.last_used_vertex 
                    other_server_reg_num = indexed_command.other_arg_server_num(extended_qubit.qubit)
                    vertex.connect_to_server(other_server_reg_num)
                    
                indexed_command.add_vertex(vertex)
                vertex.add_indexed_command(indexed_command)

                if len(indexed_command.get_vertices()) == 2: # Once both the vertices for the CZ are added, we can add the edge to the graph
                    other_vertex = [other_vertex for other_vertex in indexed_command.get_vertices() if other_vertex != vertex][0]
...
def add_indexed_command_to_circuit(circuit, indexed_command, bipartite_circuit):
    ...
    # 2) The IC is not packable but there are packings on the qubit which need to be terminated. -> Terminate all packings, add the IC to circuit
    # 4) The IC is packable as a 1q diagonal -> Just add it to circuit
    # 5) The IC is packable as a 1q antidiagonal -> Add to circuit and add correctional X gate to all current packings
    # 6) The IC is packable as a 2q CZ -> Add the CZ to the packing on the appropriate l_qubit.

    if indexed_command.is_1q() or indexed_command.is_local():
        vertex = indexed_command.vertices[0]
        extended_qubit = vertex.extended_qubit
        if not indexed_command.is_packable():
        ...
        elif extended_qubit.is_packing() and is_antidiagonal(indexed_command.command.op):
            for l_qubit in extended_qubit.current_l_qubits:
                circuit.X(l_qubit.qubit)
        
        circuit.add_gate(indexed_command.command.op, indexed_command.command.args)
...
def add_edge_to_graph(vertex1, vertex2, graph):
    if not(vertex1.is_on_graph or vertex2.is_on_graph):
        ...
        vertex2.set_bipartite_i([i for i in range(2) if i != vertex1.get_bipartite_i()][0])
        graph.add_nodes_from([vertex2.get_i()], bipartite = vertex2.get_bipartite_i())

    elif vertex2.is_on_graph:
        vertex1.set_bipartite_i([i for i in range(2) if i != vertex2.get_bipartite_i()][0])
        graph.add_nodes_from([vertex1.get_i()], bipartite = vertex1.get_bipartite_i())

    graph.add_edges_from([(vertex1.get_i(), vertex2.get_i())])

def to_indexed_commands(commands):
    indexed_commands = []
    for i, command in enumerate(commands):
        indexed_commands.append(IndexedCommand(i, command))
...
class CommandVertex():
    ...
    def add_indexed_command(self, indexed_command):
        self.indexed_commands.append(indexed_command)
    ...
    def get_indexed_command_indices(self):
        indices = []
        for indexed_command in self.indexed_commands:
...
class ExtendedQubit():
    def __init__(self, qubit, indexed_commands):
        self.qubit = qubit
    ...
    def get_currently_connected_servers(self):
        connected_servers_dict = {} # Maps the connected_server_reg_num -> all vertices connected to this vertex on that register
        ...
        for vertex in connected_vertices:
            if vertex.get_connected_server_reg_num() in connected_servers_dict.keys():
                connected_servers_dict[vertex.get_connected_server_reg_num()].add(vertex)
            else:
    ...
    def get_command_indices(self):
        command_indices = []
        for indexed_command in self.get_indexed_commands():
            command_indices.append(indexed_command.get_i())
...
class IndexedCommand():
    ...
    def other_arg_qubit(self, this_qubit):
        return [arg_qubit for arg_qubit in self.command.qubits if arg_qubit != this_qubit][0]
