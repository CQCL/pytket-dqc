...
def to_bipartite(circuit):
    ...
    """
    extended_commands = to_extended_commands(circuit.get_commands()) # Give each command an index to reference it by.
    next_vertex_index = 0
    graph = Graph()
    ...
    for extended_qubit in extended_qubits.values():
        for extended_command in extended_qubit.get_extended_commands():
            if extended_command.is_packable() and extended_command.is_local(): # Local packable gate
                ...
                if extended_command.other_arg_server_num(extended_qubit.qubit) in currently_connected_servers.keys():
                    ...
                    vertex = list(currently_connected_servers[extended_command.other_arg_server_num(extended_qubit.qubit)])[0]
                    ...
                    if extended_qubit.last_used_vertex.is_connected():
                        ...
                        vertex = extended_qubit.last_used_vertex 
                    other_server_reg_num = extended_command.other_arg_server_num(extended_qubit.qubit)
                    vertex.connect_to_server(other_server_reg_num)
                    
                extended_command.add_vertex(vertex)
                vertex.add_extended_command(extended_command)

                if len(extended_command.get_vertices()) == 2: # Once both the vertices for the CZ are added, we can add the edge to the graph
                    other_vertex = [other_vertex for other_vertex in extended_command.get_vertices() if other_vertex != vertex][0]
...
def add_extended_command_to_circuit(circuit, extended_command, bipartite_circuit):
    ...
    if extended_command.is_local():
        ...
        if not extended_command.is_packable(): # If not packable, we need to close off all current packings on this qubit
        ...
        elif extended_qubit.is_packing() and is_antidiagonal(extended_command.command.op):
        ...
        packed_vertex = [vertex for vertex in extended_command.vertices if vertex.i in bipartite_circuit.mvc][0] # If it's a global CZ, one of the two vertices it belongs to must be in the MVC, so select that vertex
        l_qubit = packed_vertex.link_qubit
        qubit = [qubit for qubit in extended_command.command.qubits if qubit != l_qubit.origin_extended_qubit.qubit][0]
        circuit.add_gate(extended_command.command.op, [qubit, l_qubit.qubit])
        for vertex in extended_command.vertices:
            vertex.added_extended_commands.append(extended_command)
    
    for vertex in extended_command.vertices:
        if vertex.i in bipartite_circuit.mvc and vertex.extended_commands == vertex.added_extended_commands: # If all of the commands on the vertex are packed we add an ending process
...
def add_edge_to_graph(vertex1, vertex2, graph):
    ...
    """
    if not(vertex1.is_on_graph or vertex2.is_on_graph): # Neither vertices are on the graph, so can fix which half of the graph they are on.
...
def to_extended_commands(commands):
    ...
    for i, command in enumerate(commands):
        extended_commands.append(ExtendedCommand(i, command))
...
class BipartiteCircuit():
    def __init__(self, circuit):
        ...
        self.graph, self.extended_commands = to_bipartite(circuit)
