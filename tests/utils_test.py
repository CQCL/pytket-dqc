...
def test_to_euler_with_two_hadamards():
    test_gatesets = [
        ...
        [Op.create(OpType.H), Op.create(OpType.Rz, [0.4]), Op.create(OpType.H)],
        [Op.create(OpType.H), Op.create(OpType.Rz, [0.5]), Op.create(OpType.H), Op.create(OpType.Rz, [0.6])],
        [Op.create(OpType.Rz, [0.7]), Op.create(OpType.H), Op.create(OpType.Rz, [0.8]), Op.create(OpType.H)],
        [Op.create(OpType.Rz, [0.9]), Op.create(OpType.H), Op.create(OpType.Rz, [1.0]), Op.create(OpType.H), Op.create(OpType.Rz, [1.1])]
    ]
    ...
    for test_gateset in test_gatesets:
        ...
        new_circ = Circuit(1)
        new_ops = to_euler_with_two_hadamards(test_gateset)
        for op in to_euler_with_two_hadamards(test_gateset):
            new_circ.add_gate(op, [0])

        identity_up_to_global_phase = orig_circ.get_unitary() @ new_circ.get_unitary().conj().T
        assert np.allclose(np.identity(2), identity_up_to_global_phase / identity_up_to_global_phase[0][0])